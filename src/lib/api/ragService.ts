...
interface SearchAgentQueryRequest {
  query: string;
  conversation_id?: string;
  user_id?: string;
}

interface SearchAgentQueryResponse {
  query: string;
  response: string;
...
async function askSearchAgent(
  query: string,
  conversationId?: string
): Promise<SearchAgentQueryResponse> {
  const body: SearchAgentQueryRequest = { query };
  if (conversationId) {
    body.conversation_id = conversationId;
  }

  const response = await fetch(`${FUNDS_AGENT_API_URL}/search/ask`, {
    method: "POST",
    headers: {
...
export class RAGService {
  async fetchConversations(
    ...
    const response = await conversationListConversations(limit, "cvm");
  ...
  async createNewConversation(
    ...
    return conversationCreateConversation(title, user, "cvm");
  }
