...
interface ControleDeAtivosContentProps {
  /** Fund name passed from the page-level selector. Skips the internal Plotly-based selector. */
  fundName?: string;
}

export function ControleDeAtivosContent({ fundName: fundNameProp }: ControleDeAtivosContentProps) {
  const filesInputId = useId(); // eslint-disable-line @typescript-eslint/no-unused-vars
  const filesRef = useRef<HTMLInputElement>(null);

  // When a fundName is injected from the parent, skip internal loading entirely.
  const selectedFund = fundNameProp ?? "";
  const fundReady = selectedFund.trim().length >= 2;
  ...
  useEffect(() => {
    ...
    fetch(pdfUrl)
      ...
      .then((buf) => {
        if (!cancelled) { setPdfBinaryData(buf); setPdfCurrentPage(1); }
      })
  ...
  const runJob = async () => {
    ...
    try {
      ...
      const workerBody: Record<string, unknown> = {
        bucket, jobId, fluxoKey, unidadesKeys,
  ...
  const loadRunArtifacts = async (run: ReportRun) => {
    ...
    try {
      ...
      const artifact = parseJsonText<{ configUrl: string; pdfUrl: string; pptxUrl?: string }>(
        presignRunText, "URLs do relatório", res.status,
      );
  ...
  return (
    <div className="space-y-10">
      ...
      {/* ── Parâmetros do Fundo ────────────────────────────────────────── */}
      <section aria-label="Parâmetros do fundo" className="min-w-0">
                    ...
                    <tbody className="divide-y divide-border/30">
                      {visibleIpcaRows.map((row) => (
                        <tr key={row.period} className="hover:bg-muted/30">
                          <td className="py-2 pr-8 font-medium text-foreground">{row.mes}/{row.ano}</td>
                          ...
                          <td className={cn(
                            ...
                            row.variacaoMes == null || isNaN(row.variacaoMes)
                              ...
                              : row.variacaoMes < 0 ? "text-red-400" : "text-foreground",
                          )}>
                            {row.variacaoMes == null || isNaN(row.variacaoMes)
                              ? "—"
      ...
      <div className="flex flex-col gap-8 lg:flex-row lg:items-start">
        ...
        <section aria-label="Upload de arquivos" className="min-w-0 flex-1">
          ...
          <div
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter" || e.key === " ") { e.preventDefault(); filesRef.current?.click(); }
            }}
          ...
          {fileRejectNote && (
            <p className="mt-2 text-sm text-amber-600 dark:text-amber-500" role="status">{fileRejectNote}</p>
          )}
        ...
        {BASE_URL && fundReady && (
          <section aria-label="Histórico de controles" className="min-w-0 lg:flex-1">
